...
class ControllerCatalogProduct extends Controller {
	...
	protected function getForm() {
        ...
        if (isset($this->request->post['product_associated'])) {
            $products = $this->request->post['product_associated'];
        } elseif (isset($this->request->get['product_id'])) {
            $products = $this->model_catalog_product->getProductAssociated($this->request->get['product_id']);
        } else {
            $products = array();
        }

        $data['product_associateds'] = array();

        foreach ($products as $product_id) {
            $associated_info = $this->model_catalog_product->getProduct($product_id);

            if ($associated_info) {
                $data['product_associateds'][] = array(
                    'product_id' => $associated_info['product_id'],
                    'name'       => $associated_info['name']
                );
            }
        }
    ...
    public function autocompleteProductsByCategory() {
        $json = array();
