...
class ModelCatalogProduct extends Model {
    public function install() {
        $this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "product_associated` (
			`product_id` int(11) NOT NULL,
            `associated_id` int(11) NOT NULL,
            PRIMARY KEY (`product_id`,`associated_id`)
	...
	public function copyProduct($product_id) {
		...
		if ($query->num_rows) {
			...
			$data['product_associated'] = $this->getProductAssociated($product_id);
    ...
    public function getProductAssociated($product_id) {
        $product_associated_data = array();

        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_associated WHERE product_id = '" . (int)$product_id . "'");

        foreach ($query->rows as $result) {
            $product_associated_data[] = $result['associated_id'];
        }

        return $product_associated_data;
    }
